...
import { supabase } from "../../../lib/supabase"; // المسار الصحيح 3 مستويات لورا
...
export default function AdminControl() {
    ...
    return (
      <div className="min-h-screen bg-gray-950 flex items-center justify-center p-6 text-right font-sans">
        <form onSubmit={handleLogin} className="bg-gray-900 p-8 rounded-[35px] border border-gray-800 w-full max-w-sm">
          ...
          <input type="password" placeholder="كلمة السر" className="w-full bg-black p-4 rounded-2xl mb-4 border border-gray-800 text-center outline-none" value={password} onChange={(e) => setPassword(e.target.value)} />
  ...
  return (
    <div className="min-h-screen bg-black text-white p-6 md:p-10 text-right" dir="rtl">
      <header className="flex justify-between items-center mb-10 border-b border-gray-800 pb-6">
        <h1 className="text-2xl font-black text-purple-400">Vidara Control 👑</h1>
        <div className="flex bg-gray-900 p-1 rounded-2xl">
          ...
          <button onClick={() => setActiveTab("codes")} className={`px-4 py-2 rounded-xl text-xs font-bold ${activeTab === "codes" ? "bg-blue-600" : "text-gray-500"}`}>سجل الأكواد 📑</button>
        </div>
      </header>
      ...
      {activeTab === "requests" ? (
        <div className="grid gap-4">
          {requests.map((r) => (
            <div key={r.id} className="bg-gray-900 p-6 rounded-[25px] border border-gray-800 flex flex-col md:flex-row justify-between items-center">
              <div className="w-full mb-4 md:mb-0">
                <p className="font-bold">{r.email}</p>
                <p className="text-sm text-gray-400">رقم المحول: {r.phone}</p>
              </div>
        ...
        <div className="overflow-x-auto rounded-2xl border border-gray-800">
          <table className="w-full text-sm text-right bg-gray-900/40">
            ...
            <tbody>{sentCodes.map((c) => (<tr key={c.id} className="border-t border-gray-800"><td className="p-4">{c.target_email}</td><td className="p-4 font-mono text-blue-400">{c.activation_code}</td></tr>))}</tbody>
          </table>
        </div>
      )}
    </div>
